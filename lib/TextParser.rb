...
private
  ...
  def parseRule(rule)
    TaskJuggler::Log.enter('parseRule', "Parsing with rule #{rule.name}")
    ...
    loop do
      ...
      begin
        token = nextToken
        TaskJuggler::Log << "Token: [#{token[0]}][#{token[1]}]"
      rescue TjException
      ...
      if patIdx.nil?
        ...
        returnToken(token)
        TaskJuggler::Log.exit('parseRule', "Finished rule #{rule.name}")
        return result
      end
